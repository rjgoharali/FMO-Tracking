...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/api/tracking/snapshot') {
    try {
    const rows = await env.DB.prepare(`SELECT u.id,u.employee_code,u.name,u.is_active,u.phone,u.email,u.created_at,ds.id AS session_id,ds.start_time,ds.expected_end_time,ds.actual_end_time,ds.status AS session_status FROM users u LEFT JOIN duty_sessions ds ON ds.fmo_id=u.id AND ds.status='ACTIVE' WHERE u.role='FMO' AND u.is_active=1 ORDER BY u.name`).all<any>();
